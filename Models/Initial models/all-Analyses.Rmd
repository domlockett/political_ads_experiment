...
knitr::opts_chunk$set(echo = FALSE,comment=NA, warning = F)
library(descr)
library(readr)
#library(RnBeads)
library(magick)
...
pdf("C:/Users/dl0ck/OneDrive/Summer 2021/PolAdsTransparency/politics_user_study/Graphs/messageXpriors.pdf", width= 20, height=5)
plot(messageAtt, xlab="Change in Pr(Ad considered more political)", text.size=13)
dev.copy(png,'C:/Users/dl0ck/OneDrive/Summer 2021/PolAdsTransparency/politics_user_study/Graphs/myplot.pdf')
  ...
  for(j in 1:2){#for each indx set (1=low political 2=high political)
    ...
    for(i in 1:8){#for every ad image
      ...
      data <- cbind(as.data.frame(cbind(t(as.matrix((wtd.ci(polAd[[d]][,indx],
               polAd[[d]]$WEIGHT )))))),'Iteration'=iter[d],'political'=politic[j], 'Ad'=adrow[i])
...
#
data1$face <- NA
data1$face[c(1:4,9:12, 17:20, 25:28)]<-1
data1$face[c(5:8, 13:16, 21:24, 29:32)]<-2
...
p1 <- ggplot(dat1, aes(y = Estimate, x = Ad, shape= as.factor(political),color = as.factor(political)))+
   ...
   # ylim(0,8)+
    xlab("") + ylab("")+
    theme_bw()+ 
    scale_y_continuous(limits = c(1,5), breaks =c(1,3,5),labels=c('Not at all political','3','Extremely political'))+ 
...
```{r experiments-p2,fig.width=12, fig.height=7,fig.cap="Averages and confidence intervals by image--  Power, Sierra, Colgate, SMSC"}
...
p2 <- ggplot(dat2, aes(y = Estimate, x = Ad, shape= as.factor(political),color = as.factor(political)))+
    ...
    ylim(0,7)+
    theme_bw()+ 
    scale_y_continuous(limits = c(1,8), breaks =c(1,4,7),labels=c('Not at all political','4','Extremely political'))+ 
  theme(panel.spacing.x = unit(27, "mm"),axis.text.x=element_text(angle=15, hjust=0.5, vjust=.5))+ theme(axis.text.x = element_text(size = 12),strip.text.x = element_text(size = 10),
    legend.text = element_text(size = 10),
    axis.text.y = element_text(size = 10))
p2
...
# 3. Opinions on political advertisments
